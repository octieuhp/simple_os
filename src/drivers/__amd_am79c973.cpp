    ...
    busControlRegisterPort(dev->portBase + 0x16)
{
    ...
    busControlRegisterPort.Write(0x102);
    ...
    //initBlock
    initBlock.mode = 0x0000; // promisounst mode = falsew
    ...
    /* printf("----");
    printfHex32((uint32_t)&sendBufferDescrMemory[0]);
    printf("--");
    printfHex32(((uint32_t)&sendBufferDescrMemory[0]) + 15);
    printf("--");
    printfHex32((uint32_t)&sendBufferDescrMemory[0] + 15);
    printf("--");
    printfHex32((uint32_t)(&sendBufferDescrMemory[0] + 15));
    printf("sendADD");
    printfHex32((uint32_t)sendBufferDescr); */
    ...
    printf("recvADD");
    printfHex32((uint32_t)recvBufferDescr);
...
uint32_t amd_am79c973::HandleInterrupt(uint32_t esp)
{
    ...
    if((temp & 0x2000) == 0x2000) printf("AMD am79c973 COLISION ERROR\n");
    if((temp & 0x1000) == 0x1000) printf("AMD am79c973 MISSED FRAME\n");
    if((temp & 0x0800) == 0x0800) printf("AMD am79c973 MEMORY ERROR\n");
    if((temp & 0x0400) == 0x0400) Receive();//printf("DATA RECEIVED\n");
    if((temp & 0x0200) == 0x0200) printf("AMD am79c973 DATA SEND\n");
...
/*     printf("\n\n");
    for(uint8_t* src = buffer + size - 1; src >= buffer; src--)
        printfHex(*src);
 */
...
{
    printf("AMD am79c973 DATA RECVED\n");
    printfHex32(recvBufferDescr[currentRecvBuffer].flags);
    printf("ennnnddddddddddddd");
